proc create_ipi_design { offsetfile design_name } {
	...
	connect_bd_intf_net [get_bd_intf_pins master_0/M_AXI_LITE] [get_bd_intf_pins Accelerometer_0/Accelerometer_AXI]

	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins master_0/M_AXI_LITE_ACLK] [get_bd_pins Accelerometer_0/Accelerometer_AXI_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins master_0/M_AXI_LITE_ARESETN] [get_bd_pins Accelerometer_0/Accelerometer_AXI_ARESETN]
	...
	set offset_file "${bd_path}/Accelerometer_v1_0_tb_include.vh"
	set fp [open $offset_file "w"]
	puts $fp "`ifndef Accelerometer_v1_0_tb_include_vh_"
	puts $fp "`define Accelerometer_v1_0_tb_include_vh_\n"
	...
	puts $fp "`define Accelerometer_AXI_SLAVE_ADDRESS ${offset_hex}"

	puts $fp "`endif"
	close $fp
}

set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores xilinx.com:user:Accelerometer:1.0]]]]
set test_bench_file ${ip_path}/example_designs/bfm_design/Accelerometer_v1_0_tb.v
...
for { set i 1 } { 1 } { incr i } {
	set design_name "Accelerometer_v1_0_bfm_${i}"
...
remove_files -quiet -fileset sim_1 Accelerometer_v1_0_tb_include.vh
import_files -fileset sim_1 -norecurse -force $interface_address_vh_file
set_property top Accelerometer_v1_0_tb [get_filesets sim_1]
